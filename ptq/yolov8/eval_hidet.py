...
det_infer = DetInfer("weights/yolov8n.hidet", "hidet")
...
for img_idx in range(len(images)):
    ...
    for result in results:
        ...
        ann_idx += 1
t_end = time.time()
logger.info(f"hidet inference time: {t_end - t_start} for {len(images)} images.")

with open("./hidet_res.json", "w") as fp_out:
    json.dump(detection_out_dict, fp_out, ensure_ascii=False, indent=4)


util.print_coco_map(config.coco_anno_file, "./hidet_res.json")